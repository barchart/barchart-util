...
public final class IntegerConverter implements ValueConverter {

	@Override
	public void applyBindings(Binder binder, String key, ConfigValue value) {
		if (value.valueType() == ConfigValueType.NUMBER) {
			binder.bind(Integer.class).annotatedWith(Names.named(key)).toInstance(((Number) value.unwrapped()).intValue());
		}
	}
}
