...
public final class ShortConverter implements ValueConverter {

	@Override
	public void applyBindings(Binder binder, String key, ConfigValue value) {
		if (value.valueType() == ConfigValueType.NUMBER) {
			binder.bind(Short.class).annotatedWith(Names.named(key)).toInstance(((Number) value.unwrapped()).shortValue());
		}
	}
