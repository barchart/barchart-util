...
public final class BooleanConverter implements ValueConverter {

	@Override
	public void applyBindings(Binder binder, String key, ConfigValue value) {
		if (value.valueType() == ConfigValueType.BOOLEAN) {
			binder.bind(Boolean.class).annotatedWith(Names.named(key)).toInstance((Boolean) value.unwrapped());
		}
	}
