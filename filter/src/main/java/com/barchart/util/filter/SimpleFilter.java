 ...
 * under the License.
 */
package com.barchart.util.filter;
...
public class SimpleFilter {
	...
	public SimpleFilter(final String attr, final Object value, final int op) {
		m_name = attr;
		m_value = value;
		m_op = op;
	}
	...
	private static String toDecodedString(final String s, final int startIdx,
			final int endIdx) {
		final StringBuffer sb = new StringBuffer(endIdx - startIdx);
		boolean escaped = false;
		for (int i = 0; i < (endIdx - startIdx); i++) {
			final char c = s.charAt(startIdx + i);
			if (!escaped && (c == '\\')) {
				escaped = true;
			} else {
				escaped = false;
	...
	public static SimpleFilter parse(final String filter) {
		...
		while (idx < filter.length()) {
			...
			if (!isEscaped && (filter.charAt(idx) == '(')) {
				...
				if (filter.charAt(idx) == '&') {
					final int peek = skipWhitespace(filter, idx + 1);
				...
				final Object top = stack.remove(0);
	...
	private static SimpleFilter subfilter(final String filter, int startIdx,
			final int endIdx) {
		...
		for (int i = 0; i < (endIdx - startIdx); i++) {
			final char c = filter.charAt(startIdx + i);
		...
		final String attr = filter.substring(startIdx, attrEndIdx);
		...
		if (op == EQ) {
			final String valueStr = filter.substring(startIdx, endIdx);
			final List<String> values = parseSubstring(valueStr);
	...
	public static List<String> parseSubstring(final String value) {
	...
	public static String unparseSubstring(final List<String> pieces) {
		final StringBuffer sb = new StringBuffer();
	...
	public static boolean compareSubstring(final List<String> pieces,
			final String s) {
	...
	private static int skipWhitespace(final String s, int startIdx) {
